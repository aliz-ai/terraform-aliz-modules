...
resource "google_container_cluster" "cluster" {
  ...
  monitoring_service          = var.monitoring_config != null ? null : var.logging_config == null ? var.monitoring_service : null
  ...
  dynamic "cluster_autoscaling" {
    ...
    content {
      ...
      dynamic "auto_provisioning_defaults" {
        for_each = var.auto_provisioning_defaults != null ? [var.auto_provisioning_defaults] : []
        iterator = defaults
        content {
          boot_disk_kms_key = defaults.value.boot_disk_kms_key == "" ? null : defaults.value.boot_disk_kms_key
          image_type        = defaults.value.image_type == "" ? null : defaults.value.image_type
          oauth_scopes      = defaults.value.oauth_scopes == [] ? null : defaults.value.oauth_scopes
